/**
 * Node.js script to create MySQL database "Employee DB",
...
connection.connect((err) => {
  ...
  // Create database "Employee DB" (with space, enclosed in backticks)
  connection.query('CREATE DATABASE IF NOT EXISTS `Employee DB`', (err) => {
    if (err) {
      console.error('Error creating database:', err);
      connection.end();
      return;
    }
    console.log('Database "Employee DB" created or already exists.');

    // Use the database
    connection.changeUser({ database: 'Employee DB' }, (err) => {
      if (err) {
        console.error('Error switching to "Employee DB":', err);
        connection.end();
        return;
      }
